package com.knight.upms.client.shiro.session;

import com.knight.common.util.RedisUtil;
import com.knight.upms.client.util.SerializableUtil;
import com.knight.upms.common.UpmsConstant;
 ...
 * Created by shuknight on 2017/2/23.
 */
public class UpmsSessionDao extends CachingSessionDAO {
    ...
    protected Serializable doCreate(Session session) {
        ...
        RedisUtil.set(KNIGHT_UPMS_SHIRO_SESSION_ID + "_" + sessionId, SerializableUtil.serialize(session), (int) session.getTimeout() / 1000);
    ...
    protected Session doReadSession(Serializable sessionId) {
        String session = RedisUtil.get(KNIGHT_UPMS_SHIRO_SESSION_ID + "_" + sessionId);
    ...
    protected void doUpdate(Session session) {
        ...
        // 更新knight_UPMS_SERVER_SESSION_ID、knight_UPMS_SERVER_CODE过期时间 TODO
    ...
    protected void doDelete(Session session) {
        ...
        if ("client".equals(upmsType)) {
            ...
            String code = RedisUtil.get(KNIGHT_UPMS_CLIENT_SESSION_ID + "_" + sessionId);
            Jedis jedis = RedisUtil.getJedis();
            jedis.del(KNIGHT_UPMS_CLIENT_SESSION_ID + "_" + sessionId);
            jedis.srem(KNIGHT_UPMS_CLIENT_SESSION_IDS + "_" + code, sessionId);
            jedis.close();
        }
        if ("server".equals(upmsType)) {
            ...
            // 清除code校验值
            RedisUtil.remove(KNIGHT_UPMS_SERVER_CODE + "_" + code);
            // 清除所有局部会话
            Jedis jedis = RedisUtil.getJedis();
            ...
            _log.debug("当前code={}，对应的注册系统个数：{}个", code, jedis.scard(KNIGHT_UPMS_CLIENT_SESSION_IDS + "_" + code));
            jedis.close();
            // 维护会话id列表，提供会话分页管理
    ...
    public Map getActiveSessions(int offset, int limit) {
        ...
        Jedis jedis = RedisUtil.getJedis();
        // 获取在线会话总数
        long total = jedis.llen(KNIGHT_UPMS_SERVER_SESSION_IDS);
        // 获取当前页会话详情
        List<String> ids = jedis.lrange(KNIGHT_UPMS_SERVER_SESSION_IDS, offset, (offset + limit - 1));
        List<Session> rows = new ArrayList<>();
        for (String id : ids) {
            String session = RedisUtil.get(KNIGHT_UPMS_SHIRO_SESSION_ID + "_" + id);
            // 过滤redis过期session
            if (null == session) {
                RedisUtil.lrem(KNIGHT_UPMS_SERVER_SESSION_IDS, 1, id);
                total = total - 1;
                continue;
            }
    ...
    public int forceout(String ids) {
        ...
        for (String sessionId : sessionIds) {
            ...
            RedisUtil.set(KNIGHT_UPMS_SHIRO_SESSION_ID + "_" + sessionId, SerializableUtil.serialize(upmsSession), (int) upmsSession.getTimeout() / 1000);
        }
